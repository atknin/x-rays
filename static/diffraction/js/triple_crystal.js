...
$("#compute").click(function(){
  ...
  $('#getX2').click();
    ...
    compute_dict['schem'] = 'triple_crystal';
  ...
  if (flag != true) {
    ...
    $.post("/polarizability/compute/", compute_dict_X ,function(data) {
      ...
      if (parseFloat(data.bragg) > parseFloat($('#fi_prmtr_'+cryst_num).val())){
        compute_dict['fi_'+cryst_num] = parseFloat($('input[name=assym_alfa_then_beta_'+cryst_num+']').filter(':checked').val()) *  Math.abs($('#fi_prmtr_'+cryst_num).val());
