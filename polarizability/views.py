...
# Собственная кривая кристалла -–––––––––––––––––––––––––––––––––––
		...
		for i in range(0,10000):
			...
			prover = (1/4/gamma_0)*(X0*(b+1)+b*alfa+cmath.sqrt(((X0*(1-b)-b*alfa)*(X0*(1-b)-b*alfa))+4*b*math.pow(C,2)*(math.pow(Xh.real,2)-math.pow(Xh.imag,2)-2j*Xh.real*Xh.imag)))
			if prover.imag < 0:
				eps = (1/4/gamma_0)*(X0*(b+1)+b*alfa-cmath.sqrt(((X0*(1-b)-b*alfa)*(X0*(1-b)-b*alfa))+4*b*math.pow(C,2)*(math.pow(Xh.real,2)-math.pow(Xh.imag,2)-2j*Xh.real*Xh.imag)))
			else:
				eps = prover
			R=(2*eps-X0)/Xh/C
